...
int Player::autoPlayCard() {
    return board.drawCardFromMainDeck();
}
...
size_t Player::getPlayedCardsCount() const {
    return board.getPlayedCardsCount();
}

size_t Player::getRoundsWon() const {
    return board.getRoundsWon();
}

bool Player::isStanding() const {
    return board.isStanding();
}
...
void Player::boardStatusMessage(const int opponentScore) const {
    cout << "Your score: "
         ...
         << endl
         << "Your board: "
         << (board.showCardsPlayed().empty() ? "No cards played so far." : board.showCardsPlayed())
         << endl;
}

void Player::playerIsStandingMessage() const {
    cout << getName()
         << " is standing."
         << endl;
}

void Player::isPassingTurnMessage() const {
    cout << name
         << " is passing their turn."
         << endl;
}

void Player::standPrompt() const {
    cout << "Would you like to Stand([S]tand/[Y]es/[N]o)?"
         << endl;
}

void Player::openerMessage(int opener) const {
    cout << "Player "
         << name
         << "'s opener:"
         << opener
